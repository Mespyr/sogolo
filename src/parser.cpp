...
namespace Parser {
    ...
    std::string PARSING_ERROR_NO_TOKEN::error() {
        std::string error_msg = "Parsing Error on line " + std::to_string(line_number) + ".\n";
        error_msg += "  " + line + "\n";
        error_msg += msg + "\n";
        return error_msg;
    }
    ...
    std::string Parser::get_error() {
        ...
        if (ErrorType == REGULAR_NO_TOKEN)
            return no_token_parsing_error.error();
        else
        ...
        for (int i = 0; i < vec.size(); i++) {
            ...
            if (curnode.type == AST_ATOM) {
                ...
                if (curline.nodes.size() > 0) {
                    ...
                    error_found = true;
                    ErrorType = REGULAR_NO_TOKEN;
                    no_token_parsing_error = PARSING_ERROR_NO_TOKEN{"Unexpected '[' token found while parsing.", curnode.line_number, curnode.line};
                    return curnode;
                }
            }
